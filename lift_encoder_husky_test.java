...
@Autonomous(name = "HUSKY TEST FINAL NEW 14", group = "Robot")
public class husky_lens_test extends LinearOpMode {
        ...
        while (opModeIsActive()) {
            ...
            for (int i = 0; i < blocks.length; i++) {
                ...
                if (block.id == 1 || block.id == 2) {
                    ...
                    if (block.x > thirdWidth && block.x < 2 * thirdWidth) {
    ...
    public void liftEncoderDrive (double speed, double left_lift_inches, double right_lift_inches, double timeouts){
        ...
        if (opModeIsActive()) {
            ...
            runtime.reset();
            liftLeftMotor.setPower(Math.abs(speed / 2));
            liftRightMotor.setPower(Math.abs(speed / 2));
